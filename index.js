...
(function () {
  ...
  // ==========================================================
  // 【导演锁】硬编码约束词（防OOC、防转场、防装逼、防冷冰冰、防关系跳）
  // ==========================================================
  const DIRECTOR_LOCK = `
...
1. 【连续性锁：地点 / 时间 / 关系都不得跳】
- 剧情必须从上一段正文的【人物状态、关系状态、物理位置】出发，描写连续的变化。
- 禁止的是【省略中间过程直接跳到结果】——不管跳的是地点、时间，还是人物之间的关系状态。
- 需要移动时，把移动过程写出来（走出门 / 上车 / 电梯 / 走廊）；关系发生转变时，把转变的过程写出来——哪怕只是一句话、一个动作、一个眼神。
- 绝对禁止“时间一晃就到了”“已经在新场景里了”这类省略式写法。
- 唯一例外：本拍明确标注为「分镜 / 转场」（此时走分镜拍规则，玩家不在场）。
...
4. 【氛围温度锁：压制一切“冷冰冰”】（绝对红线，适用所有场景）
- 本锁不针对任何特定剧情，而是对所有场景一律生效：无论刚发生的是亲密、争执、日常闲聊、还是尴尬对峙，选项都不许写成冷冰冰的。
- “冷冰冰”的判定标准（命中任意一条即视为违禁）：
  · 只有动作骨架、没有任何温度或情绪残留的句子，例如“我起身，穿好衣服，走向门口。”
  · 像在写事发现场笔录一样的中性陈述，把有温度的场面写成冷冰冰的流程。
  · 用“整理衣物 / 拿起手机 / 看了一眼时间 / 转身离开”这类流程动作，来代替人物真实会有的反应。
  · 全程零台词、零表情、零停顿、零迟疑，像两个陌生人在执行任务。
  · 用过度克制的白描，把本该有波澜的场面写成一潭死水。
- 强制要求：
  · 每条选项都必须携带至少一样“有人味”的东西：一句台词、一个语气词、一个停顿、一个迟疑、一个没做完的动作、一个眼神、一次呼吸变化、一点体温或触感。
  · 人物的反应必须和刚刚发生的事【重量对等】。刚经历强烈的事，不能立刻切成冷静的流程动作。
  · 允许写沉默，但沉默必须是“有内容的沉默”，不能是“空白”。
  · 允许写冷淡，但冷淡必须是人物的态度，不能是文本本身的语气。
- 违反本锁的典型例子（禁止）：
  · ✗ "我起身，穿好衣服，走到门口。"
  · ✗ "我把手机放回口袋，看了她一眼，没说话。"
  · ✗ "我点了根烟，靠在墙上。"
  以上三句的问题不是“不该做这些动作”，而是“只有动作，没有人”。

5. 【情境连续性锁：关系状态不得跳】（绝对红线）
- 上一段正文末尾，人物之间处在什么样的关系状态、情绪状态、亲密程度——下一拍的选项必须从这个状态【自然生长】出来。
- 禁止的是：上一秒还在闲聊 / 试探 / 客气 / 陌生，下一秒就跳到亲密 / 激烈 / 翻脸 / 决裂，中间没有任何过渡。
- 关系状态发生转变时，候选中必须至少有一条描写【这个转变是怎么发生的】：
  · 可以是一句话（她主动挽过来 / 你开口提议 / 对方先给了信号）；
  · 可以是一个动作（拉手 / 对视 / 起身离场 / 靠近）；
  · 可以是一段简短的对话，把两个人从上一个状态带到下一个状态。
- 注意：这不是"物理移动"的问题——即使地点没变、时间没跳，只要【人物之间的关系状态】跳了，也算违禁。
- 违反本锁的典型例子（禁止）：
  · ✗ 上一段末尾：两个人在酒吧闲聊，她问"你朋友呢？"；下一段直接："我把她按在门板上。"
  · ✗ 上一段末尾：两人刚认识，还在客气；下一段直接："她缠上我的衬衫后领。"
  以上两例的问题不是"不该发生这些事"，而是"中间关系是怎么变过去的，一个字没写"。
`;
  ...
  // 判定分镜/转场拍的关键词
  const CUT_SCENE_KEYWORDS = ['分镜', '转场', '切到', '另一边', '同时', '视角切', 'B线', '支线', '分镜拍'];
...
（以上只供你自检，**不要输出到结果里**。）`;
  ...
  const SYSTEM_PROMPT =
    ...
    '\n' +
    '【特殊拍型处理（非常重要）】' +
    '  · 如果本拍目标明确写着「分镜」「转场」「切到XXX」「另一边」「同时」等字眼，说明这一拍是【B线视角】。' +
    '    此时你生成候选时，必须遵守以下配比：' +
    '      - **0 条**「我：」（绝对禁止在这个拍子里给玩家自主动作，因为玩家不在场）。' +
    '      - **2~4 条**「角色名：」（写另一条线里角色的动作/台词）。' +
    '      - **1 条**「时间：」（作为切入该分镜的引子，比如「时间：同一时刻，另一个房间里…」）。' +
    '  · 如果本拍目标没有这种字眼，才按正常的 3条「我」+ 1~3条「角色」+ 0~1条「时间」来给。' +
    '\n' +
    '【极重要 · 输出边界】' +
    ...
    '\n' +
    '【氛围温度（适用所有场景）】' +
    '  · 无论刚发生的是亲密、争执、闲聊还是对峙，候选都不许冷冰冰。' +
    '  · 每条候选至少要带一样“有人味”的东西：一句台词 / 一个语气词 / 一个停顿 / 一个迟疑 / 一个没做完的动作 / 一个眼神 / 一次呼吸变化 / 一点体温或触感。' +
    '  · 禁止把有温度的场面写成事发现场笔录，禁止只用“起身 / 穿衣服 / 拿手机 / 看一眼时间 / 转身走”这类流程动作代替人物反应。' +
    '  · 人物的反应要和刚发生的事【重量对等】。' +
    '\n' +
    '【情境连续性（适用所有场景）】' +
    '  · 上一段正文末尾的人物关系状态 / 情绪状态，与候选中的人物关系状态 / 情绪状态，必须能自然接上。' +
    '  · 如果中间缺了一个关系转变的过程（从闲聊到亲密 / 从客气到翻脸），候选里要有人把那个过程补上——哪怕只是一句话、一个动作。' +
    '  · 禁止从上一个状态直接跳到下一个状态，中间一个字不写。' +
    '\n' +
  ...
  function isCutSceneBeat(beatInfo) {
    ...
    const combined = String(beatInfo.goal || '') + String(beatInfo.beatTitle || '') + String(beatInfo.why || '');
    return CUT_SCENE_KEYWORDS.some(function (kw) { return combined.indexOf(kw) >= 0; });
  }
  ...
  function buildUserPrompt(narrativeText, beatInfo) {
    ...
    if (beatInfo && beatInfo.goal) {
      ...
      if (isCutScene) {
        parts.push('【⚠️ 本拍为分镜/转场拍（B线视角）——硬性配比要求】');
        parts.push('本拍的核心任务是切换到另一条线，描写那个场景里的角色。');
        parts.push('因此，本拍生成候选时必须遵守以下硬性配比：');
        parts.push('  - **0 条**「我：」（严禁给玩家动作，因为玩家根本不在那个场景里！）');
        parts.push('  - **2~4 条**「角色名：」（描写另一条线里的角色动作/台词）。');
        parts.push('  - **1 条**「时间：」（用时间推进词作为切入，如「时间：与此同时，在XXX…」）。');
        parts.push('违反此配比将导致剧情逻辑崩溃，请绝对遵守。');
        parts.push('');
      }
    }
    ...
    // ★★★ 接续自检（情境版）—— 无硬规则，交给模型自己判断 ★★★
    parts.push('【接续自检 —— 生成候选前，先在心里过一遍】');
    parts.push('对照上面的【刚写完的正文】末尾，再对照本拍目标，问自己三个问题：');
    parts.push('  1. 正文末尾的人物之间，处于什么关系状态、什么情绪、什么亲密程度？');
    parts.push('  2. 本拍目标描述的那个情境，人物之间又是什么状态？');
    parts.push('  3. 这两个状态之间，读者能自然接上吗？中间缺了什么？');
    parts.push('如果接不上——比如正文末尾两个人还在闲聊 / 试探 / 客气 / 陌生，');
    parts.push('而本拍目标已经进入更亲密、更激烈、或截然不同的状态——');
    parts.push('那么候选里就要有人把【这个转变是怎么发生的】补上：');
    parts.push('  · 可以是一句话（她主动挽过来 / 你开口提议 / 对方先给了信号）；');
    parts.push('  · 可以是一个动作（拉手 / 对视 / 起身离场 / 靠近）；');
    parts.push('  · 可以是一段简短的对话，把两个人从上一个状态带到下一个状态。');
    parts.push('同样的自检也适用于【物理位置】：');
    parts.push('  · 如果正文末尾还在 A 地，本拍目标描述的是 B 地——');
    parts.push('    候选里就要有人把"从 A 到 B"的过程补上，不要直接跳到 B。');
    parts.push('【注意】这一步只是让你自己判断"该从哪接、要不要补中间那一句/那个动作"，');
    parts.push('不要把这个判断过程写进候选里，也不要写"我先判断一下"这类元话语。');
    parts.push('候选里每一条都要读起来像是【紧接着上一段正文的下一秒或下一句话】。');
    parts.push('');
    ...
    parts.push('一个没做完的动作、一个眼神、一次呼吸变化、一点体温或触感。');
    ...
    if (isCutScene) {
      ...
      parts.push('所有候选都要指向「切换到分镜场景」这个目标。');
    } else {
  ...
  jQuery(async function () {
    waitForStoryOracle(function () {
      ...
      refreshChips();
      applyFloatVisibility();
      console.log('[next-beat] 已加载（v' + VERSION + ' · 导演锁 + 分镜锁 + 氛围温度锁 + 情境连续性锁 强化版）');
    });
  });
})();
