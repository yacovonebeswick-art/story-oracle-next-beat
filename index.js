...
// v3.9.2
//   · v3.9.2 调整候选数量与配比：恒定 3 条「我」+ 1~3 条「角色X」+ 0~1 条「时间」；
//            其他角色的候选填入输入框时自动加「角色名：」前缀（我 / 时间 不加）。
...
(function () {
  ...
  const VERSION = '3.9.2';
  const CFG_VERSION = 14;

  const ORACLE_SETTINGS_KEY = 'storyOracle';

  // ----------------------------------------------------------------------------
  // 内置默认「选项模板 DIY」——MBTI 八维选路池
...
## Vol.4 八维功能——选项的选路池
本轮按配比从 8 个维度里挑出足够条数（挑的维度数 = 本轮条数），哪几维在当下最成立就出哪几维。
...
**注意**：维度只用于你内部选路；**选项正文里绝对不许出现 MBTI 名称**（"Ne / Fe / Se / Te / Ni / Fi / Si / Ti"或"外倾直觉"这类字样）。
...
- **时间条**（若有）：以时间推进 / 换场为主，一句话交代一个新的场景或时间点。
- **不写对方反应**：不写对方脸色、语气、沉默、顿住、退一步。
- **不写内心独白**：不写"我心里想……"、"我知道……"。
- **人设决定长短**：
  - 话痨 / 直率 / 情绪外露型：允许 2~3 句连说，可以有迟疑、可以自己打断自己、可以一激动就偏题——但方向必须仍是"迈出的一步"。
...
- **3 条** 以 \`我：\` 开头（玩家自己的动作 / 台词）；
- **1~3 条** 以 \`角色名：\` 开头（正文里对该角色的称呼，例如 \`王奕：\` 或 \`吕子乔：\`；不要写"角色A"这种占位名，除非确实不知道该怎么称呼）；
- **0~1 条** 以 \`时间：\` 开头（仅当本拍目标明确需要换场 / 换时间时）。
**总数 4~7 条**。
...
【6 条示范：3 条我 + 2 条角色 + 1 条时间】
我：我眼睛一亮，扯到别的地方去："诶你这么一说，我想到另一个事——去年那回不也是差不多的情况吗？"
我：我凑过去一点，先接他的话："行行行你先别急，先听我说完——"
我：我一把把椅子拉开，站起来："算了别说了，我们出去走走。"
吕子乔：他把手机往桌上一扔，靠着沙发笑了一声。
陈美嘉：她把手里的包往肩上一甩，扭头就想走。
时间：半小时后，天色完全暗了下来。
...
- ✗ "我按计划收回视线。"（掌控全局感）
- ✗ "A. 我以 Ne 为主导发散了一下……"（出现 MBTI 字样 / 编号）
- ✗ "角色A：他抬起头。"（用了占位名而不是正文里的称呼）

## Vol.8 终检（仅供你自己在脑内过一遍，**不要输出到结果里**）
1. 数量：是不是 3 条「我」+ 1~3 条「角色X」+ 0~1 条「时间」，总数 4~7？
2. 行首格式：每条都是 \`标签：内容\` 吗？「我」条用 \`我：\`，「时间」条用 \`时间：\`，「角色」条用的是正文里的称呼吗？
3. 接续：这条选项与上一段末句是同一口气吗？
4. 信息源：这条选项里关键信息，说话/行动的人凭什么是知道的？
5. 预知感：有没有"我早就知道/我早料到/果然如此"这类姿态？
6. 零反应：面对冲击时的动作是不是只剩"没反应"？有没有写出一样具体在动的东西？
7. 点破：台词有没有一句替对方下结论？
8. 占上风：选项里有没有替对方写反应？
9. 稳：选项有没有默认写"稳如老狗"、游刃有余？
10. 掌控：选项有没有带出"我早就安排好、他不过是棋子、节奏在我手上"这类支配感？
11. 长短：每条的长度是否符合人设？
12. 维度：每条对应一个维度吗？有没有两条同维？
13. MBTI 字样：选项正文里有没有漏出 MBTI 字样或中文维度名？
（以上十三问只供你自检，**不要输出到最终结果里**。）`;
  ...
  // v3.9.2：系统提示词——候选配比与行首格式换成「我：」/「角色名：」/「时间：」。
  const SYSTEM_PROMPT =
  ...
  // v3.9.2：标签合法判定——「我」「时间」「选项」「角色A..C」「人名（中文/英文/数字/下划线，1~12 字）」都放行。
  // 英文名（如「Alan」「MrSmith」）现在也能过；避免误吃「Vol.4」这类小节标题（已在 isJunkLine 里挡掉）。
  ...
  // v3.9.2：优先解析「标签：内容」行（中英文冒号都吃，允许 **加粗** 包裹）。返回 [{label, content}]。
