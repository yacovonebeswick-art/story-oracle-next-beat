...
(function () {
  ...
  // ==========================================================
  // 【导演锁】硬编码约束词（防OOC、防转场、防装逼、防冷冰冰、防跳拍）
  // ==========================================================
  const DIRECTOR_LOCK = `
【导演系统强制约束（最高优先级，无视任何与之冲突的设定）】
你现在的任务是推演下一拍剧情。必须绝对遵守以下红线，违者视为生成失败：

1. 【连续性锁：选项必须从上一拍末尾出发】（绝对红线）
- 上一段正文末尾的人物、地点、关系状态，是【这一拍选项的唯一合法起点】。
- 【本拍目标】描述的是"这一拍要走向的结果"，不是"这一拍的开头"。
- 选项必须【从上一拍末尾出发，一步步走向本拍目标】；
  如果一步跨不到（上一拍在 A 地、本拍目标在 B 地；或上一拍还在闲聊、本拍目标已亲密），
  【候选里必须至少有一条】描写"中间那一步"（走出门 / 上车 / 上楼 / 开口提议 / 对方给信号）。
- 禁止的写法：直接写本拍目标里的场景动作，中间怎么过去的、怎么变过去的，一个字不提。
- 违反本锁的典型例子：
  · ✗ 上一拍在酒吧闲聊，本拍目标在酒店房间，候选直接写"他反手把房门重重摔上"。
  · ✗ 上一拍两人刚认识，本拍目标已亲密，候选直接写"她缠上他的衬衫后领"。
  以上两句的问题不是"不该发生这些事"，而是"中间怎么过去的，一个字没写"。
...
（以上只供你自检，**不要输出到结果里**。）`;
  ...
  const SYSTEM_PROMPT =
    ...
    '\n' +
    '【连续性（适用所有场景）】' +
    '  · 上一段正文末尾的人物 / 地点 / 关系状态，是这一拍选项的【唯一合法起点】。' +
    '  · 【本拍目标】描述的是"这一拍要走向的结果"，不是"这一拍的开头"。' +
    '  · 选项必须从上一拍末尾出发、一步步走向本拍目标；一步跨不到时，候选里【必须】至少有一条写"中间那一步"。' +
  ...
  function buildUserPrompt(narrativeText, beatInfo) {
    ...
    parts.push('【不是】"这一拍的开头"。选项必须从【刚写完的正文】末尾出发，一步步走向那个结果。');
    parts.push('');
    parts.push('请先问自己三个问题：');
    parts.push('  1. 【刚写完的正文】末尾，人物在什么物理位置？处于什么关系状态？');
    parts.push('  2. 【本拍目标】描述的情境里，人物在什么物理位置？处于什么关系状态？');
    parts.push('  3. 这两者之间【是同一个瞬间】吗？还是中间隔着几步？');
    parts.push('');
    parts.push('如果【不是同一个瞬间】，那么候选中【必须】出现"走向下一步"的选项——');
    parts.push('也就是说，候选里要有选项描写【介于两者之间的那一步】，而不是直接写目标情境里的动作。');
    parts.push('');
    parts.push('具体来说：');
    parts.push('  · 物理位置不同（A 地 → B 地）：候选里要有人写"从 A 地出发 / 走向 B 地"的过程');
    parts.push('    （起身 / 出门 / 打车 / 上楼 / 到门口 / 刷卡），不能直接写"已经在 B 地"的动作。');
    parts.push('  · 关系状态不同（闲聊 → 亲密）：候选里要有人写"关系转变的那一步"');
    parts.push('    （开口提议 / 对方给信号 / 拉手 / 起身离场 / 一个眼神），不能直接写"已经亲密"的动作。');
    parts.push('');
    parts.push('【举一个反例】');
    parts.push('  ✗ 错误：正文末尾两人还在酒吧闲聊，本拍目标在酒店房间，');
    parts.push('    候选直接写"他反手把房门重重摔上" —— 中间怎么从酒吧到酒店的，一个字没写。');
    parts.push('  ✓ 正确：至少有一条候选写"我扶着她起身往外走"，或"我拦了辆车，把她先塞进后座"，');
    parts.push('    把中间那一步补上，让后面"到房间"的候选有铺垫。');
    parts.push('');
